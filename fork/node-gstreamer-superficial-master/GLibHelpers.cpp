...
Local<Value> gvalue_to_v8(const GValue *gv)
{
	...
	else if (GST_VALUE_HOLDS_SAMPLE(gv))
	{
		...
		if (gcaps)
		{
			...
			if (structure)
				gst_structure_to_v8(caps, structure);
		}
...
void v8_to_gvalue(Local<Value> v, GValue *gv, GParamSpec *spec)
{
	if (v->IsNumber())
	{
		auto number = v->ToNumber(Nan::GetCurrentContext()).ToLocalChecked();
		if (number->IsInt32())
		{
			g_value_init(gv, G_TYPE_INT);
			g_value_set_int(gv, number->ToInteger(Nan::GetCurrentContext()).ToLocalChecked()->Value());
		}
		else
		{
			g_value_init(gv, G_TYPE_FLOAT);
			g_value_set_float(gv, number->Value());
		}
	}
	else if (v->IsString())
	{
